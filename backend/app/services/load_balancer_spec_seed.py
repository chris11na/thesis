"""Ensure load balancer catalog filter parameters exist and populate spec values."""
...
from app.services.vlb_equipment_parser import (
    VLB_EQUIPMENT_SPEC_PARAMETERS,
    VLB_TYPE_CODE,
    parse_vlb_equipment_product,
)
...
def _load_balancer_product_ids(db: Session) -> set[int]:
    rows = (
        db.query(Product.id)
        ...
        .filter(EquipmentGroup.code == "load_balancer")
        .all()
    )
    return {int(pid) for (pid,) in rows}


def refresh_load_balancer_spec_values(db: Session) -> int:
    """Parse load balancer catalog rows and upsert structured filter spec values."""
    param_ids = _ensure_parameters(
        db, VLB_EQUIPMENT_SPEC_PARAMETERS + SUPPORT_TIER_SPEC_PARAMETERS
    )
    updated = 0
    product_ids = _load_balancer_product_ids(db)
    if not product_ids:
        return 0
