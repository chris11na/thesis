"""Ensure telephony catalog filter parameters exist and populate spec values."""
...
from app.services.telephony_equipment_parser import (
    TELEPHONY_EQUIPMENT_SPEC_PARAMETERS,
    VCM_TYPE_CODE,
    VP_TYPE_CODE,
    parse_telephony_equipment_product,
)
...
def _telephony_product_ids(db: Session) -> set[int]:
    rows = (
        db.query(Product.id)
        ...
        .filter(EquipmentGroup.code == "telephony")
        .all()
    )
    return {int(pid) for (pid,) in rows}


def refresh_telephony_spec_values(db: Session) -> int:
    """Parse telephony catalog rows and upsert structured filter spec values."""
    param_ids = _ensure_parameters(
        db, TELEPHONY_EQUIPMENT_SPEC_PARAMETERS + SUPPORT_TIER_SPEC_PARAMETERS
    )
    updated = 0
    product_ids = _telephony_product_ids(db)
    if not product_ids:
        return 0
    ...
    for product in db.query(Product).filter(Product.id.in_(product_ids)).all():
        ...
        parsed: dict[str, str] = {}
        if category in (VCM_TYPE_CODE, VP_TYPE_CODE):
