"""Ensure V-Sense management filter parameters exist and populate spec values."""
...
from app.services.vs_management_parser import (
    VS_MANAGEMENT_SPEC_PARAMETERS,
    VS_TYPE_CODE,
    parse_vs_management_product,
)
...
def _management_product_ids(db: Session) -> set[int]:
    rows = (
        db.query(Product.id)
        ...
        .filter(EquipmentGroup.code == "management")
        .all()
    )
    return {int(pid) for (pid,) in rows}


def refresh_management_spec_values(db: Session) -> int:
    """Parse management catalog rows and upsert structured filter spec values."""
    param_ids = _ensure_parameters(
        db, VS_MANAGEMENT_SPEC_PARAMETERS + SUPPORT_TIER_SPEC_PARAMETERS
    )
    updated = 0
    product_ids = _management_product_ids(db)
    if not product_ids:
        return 0
