"""Ensure server catalog filter parameters exist and populate spec values."""
...
def _server_product_ids(db: Session) -> set[int]:
    rows = (
        db.query(Product.id)
        ...
        .filter(EquipmentGroup.code == "server")
        .all()
    )
    return {int(pid) for (pid,) in rows}


def refresh_server_spec_values(db: Session) -> int:
    """Parse server support catalog rows and upsert structured filter spec values."""
    param_ids = _ensure_parameters(db, SUPPORT_TIER_SPEC_PARAMETERS)
    updated = 0
    product_ids = _server_product_ids(db)
    if not product_ids:
        return 0
